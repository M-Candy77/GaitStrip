...
class Model:
    def __init__(self,
        ...
        print(train_pid_num)
        self.encoder = gaitstrip.c3d_vgg_Fusion(num_classes=self.train_pid_num)
        ...
        self.optimizer = optim.Adam([
            {'params': self.encoder.parameters()}], lr=self.lr, weight_decay=self.weight_decay)
    ...
    def fit(self):
        ...
        for seq, view, seq_type, label, batch_frame in train_loader:
            ...
            seq = torch.stack((seq,seq,seq),dim=1)
            seq = seq.cuda()
            ...
            # print(seq.shape)
            triplet_feature,outputscse = self.encoder(seq)
            ...
            self.lossall.append(loss.data.cpu().numpy())
            ...
            if self.restore_iter % 70000 == 0:
                print('----------------LR-------------------')
                self.optimizer = optim.Adam([
                    {'params': self.encoder.parameters()}], lr=0.00001, weight_decay=self.weight_decay)  
            ...
            if self.restore_iter % 100 == 0:
                ...
                print(',cse+tri={0:.8f}'.format(np.mean(self.lossall)), end='')
